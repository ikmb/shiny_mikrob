...
rarefaction_df <- reactive({
  ...
  featureTable <- otu_table(ps())
  featureTable_t <- t(as(otu_table(ps()), "matrix"))
  depths <- seq(100, floor(max(rowSums(featureTable_t)) / 10) * 10, by = 100)

  rarefaction_results <- data.frame()
  for (d in depths) {
    rarefied <- rrarefy(featureTable_t, sample = d)
...
```{r download_rarefaction, echo=FALSE}
output$download_rarefaction <- downloadHandler(
  filename = function() {
  ...
  content = function(file) {
    ggsave(
      file,
      plot = {
        # same ggplot() call as in renderPlot above
        ggplot() +
          geom_line(data = summary_df(),
                    aes(x = Depth, y = mean_richness, color = .data[[input$group_var]]),
                    size = 1) +
...
downloadButton("download_rarefaction", "Download PNG")
```
...
numericInput("plot_height", "Height (in inches):", value = 6)
numericInput("plot_dpi", "DPI:", value = 300)
downloadButton("download_alpha_plot", "Download PNG")
```
...
output$download_alpha_plot <- downloadHandler(
  ...
  content = function(file) {
    ggsave(file, plot = alpha_div_plot_obj(), width = 12, height = 6, dpi = 300)
  }
)
```
...
numericInput("plot_height", "Height (in inches):", value = 6)
numericInput("plot_dpi", "DPI:", value = 300)
downloadButton("download_alpha_metric_plot", "Download PNG")
...
output$download_alpha_metric_plot <- downloadHandler(
  filename = function() {
...
downloadButton("download_beta_plot", "Download PNG")
...
output$relab_download_ui <- renderUI({
  ...
  tagList(
    ...
    numericInput("plot_height", "Height (in inches):", value = 6),
    numericInput("plot_dpi", "DPI:", value = 300),
    downloadButton("download_relab_plot", "Download PNG"),
...
relab_long <- reactive({
  ...
  otu_df %>%
    ...
    left_join(taxonomy_df %>% rownames_to_column(var = "Taxa"), by = "Taxa") %>%
...
# Download PNG
output$download_relab_plot <- downloadHandler(
...
output$absab_download_ui <- renderUI({
  ...
  tagList(
    ...
    numericInput("plot_height", "Height (in inches):", value = 6),
    numericInput("plot_dpi", "DPI:", value = 300),
    downloadButton("download_absab_plot", "Download PNG")
  )
})
```

```{r absab_download_plot, echo=FALSE}
output$download_absab_plot <- downloadHandler(
